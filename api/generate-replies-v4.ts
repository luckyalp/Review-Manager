...
- Falls der Grund mit "viel Betrieb" zusammenhaengt (Lautstaerke, Temperatur, Enge bei vollem Haus): bleibe bei dem einfachen Adjektiv aus der Bewertung (laut, eng, warm etc.) — keine Nominalisierung ("Laermpegel", "Enge") oder Passiv-Konstruktion ("hat den Besuch getruebt/beeintraechtigt"). Formuliere als nicht-absolute, validierende Aussage, z.B. "Kann sein, dass das fuer den einen oder anderen [laut/eng/warm] wirkt, wenn viel los ist." Dieses Template gilt NUR fuer betriebsbedingte Gruende — bei festen Regeln oder baulichen Gegebenheiten (z.B. Tischzeit-Limit, Lage eines Tisches) reicht der allgemeine sachliche Grund von oben, ohne Bezug auf "viel los ist".

...
function buildFreePrompt(reviewText: string, stars: number, reviewerName: string, settings: any, analysis?: {count: number, points: string[], categories: string[], forceSummarize: boolean, lobpunkte: string[], vorOrtErwaehnt: boolean}): string {
  ...
  // ─── KATEGORIE-BLOECKE (getrennt, damit der Texter pro Bewertung nur die ──
  // tatsaechlich relevante(n) Kategorie(n) sieht statt immer alle drei) ──────
  const KATEGORIE_A = `KATEGORIE A - KONZEPT ODER STRUKTURELL FIX: Wenn die Kritik etwas betrifft, das durch die Art des Ladens oder eine feste Regel bedingt ist und sich grundsaetzlich nicht aendert (z.B. Tischzeit-Limit, Lage eines bestimmten Tisches, Oeffnungszeiten, Lautstaerke oder Temperatur bei vollem Haus):
- Nenne einen sachlichen, neutralen Grund, WARUM es so ist. NIEMALS "andere Gaeste moegen das" (wirkt wie eine Abwertung der Kritik). NICHT entschuldigen fuer etwas, das richtig war. Bei der Regel "Tische nur fuer Gaeste mit Hauptgericht": Der Grund ist, dass dadurch mehr Gaeste insgesamt einen Platz zum Essen bekommen koennen, nicht nur "fair verteilen" als abstraktes Prinzip — z.B. "damit moeglichst viele auch wirklich bei uns essen koennen" statt nur "damit wir die Plaetze fair vergeben".
...
- ABSCHLUSS fuer Kategorie A:
  ...
  const KATEGORIE_B = `KATEGORIE B - ECHTER FEHLER, EINZELFALL: Wenn die Kritik einen objektiven Fehler beschreibt, der beim naechsten Besuch anders laufen koennte (z.B. falsches Gericht, lange Wartezeit ohne Grund, unfreundliches Personal) und NICHT durch Konzept/Policy bedingt ist:
...
- Falls der Gast NICHTS vor Ort gesagt hat: pruefe, WANN der Gast das Problem realistischerweise haette merken/melden koennen:
  ...
  const KATEGORIE_C = `KATEGORIE C - SUBJEKTIV/GESCHMACK/WAHRNEHMUNG: Wenn die Kritik eine persoenliche Geschmacks- oder Wahrnehmungssache beschreibt (z.B. "fuer meinen Geschmack zu scharf", "fuer mein Hunger etwas knapp", "war nicht so meins"), bei der NICHTS objektiv falsch gelaufen ist:
...
- WENN nichts dazu in der Bewertung steht: keine Erwaehnung.`
  ...
  // Welche Kategorien sind in dieser Bewertung tatsaechlich vorhanden (laut Agent 1)?
  // Fallback: wenn Agent 1 keine Kategorien liefert (z.B. Fehler), alle drei zeigen,
  // damit nichts verloren geht statt mit zu wenig Kontext zu schreiben.
  const cats = analysis?.categories && analysis.categories.length > 0
    ? Array.from(new Set(analysis.categories))
    : ['A', 'B', 'C']
...
UMGANG MIT MEHREREN KATEGORIEN GLEICHZEITIG: Die Kategorie(n) dieser Bewertung sind unten als eigene Bloecke aufgefuehrt (von der Vorstufe bereits erkannt). Falls mehr als ein Block vorhanden ist, gilt:
...
- Erklaere Kat-A-Sachen ehrlich als eigenen Gedanken. Bei Lautstaerke-Kritik gilt die ZWEI-FAELLE-Regel im Kategorie-A-Block (Akustik vs. Musik).

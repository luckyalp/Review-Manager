...
Beispielsätze (mehrere Formulierungen zur Auswahl, je nach Variante unterschiedlich waehlen): "Was genau passiert ist, wissen wir so nicht." / "Das können wir im Nachhinein nicht mehr nachvollziehen." / "Damit wir das einordnen können, brauchen wir mehr." / "Was da konkret schiefgelaufen ist, lässt sich im Nachhinein leider nicht mehr rekonstruieren." / "Im Nachhinein lässt sich nicht mehr genau sagen, was da bei uns nicht gestimmt hat."
...
Satz 3 ist die ehrliche Slot-3A-Einordnung — waehle EINE der Slot-3A-Beispielsaetze (siehe oben, "SLOT 3A"), mit natuerlichem Uebergang (z.B. "Ehrlich gesagt,"), als eigener klarer Satz. V2 und V3 muessen UNTERSCHIEDLICHE Formulierungen waehlen, nicht dieselbe. Baue daraus KEINEN Bandwurmsatz mit der Validierung aus Satz 1 — also NICHT "Letztendlich war's nicht in Ordnung, und das koennen wir nicht nachvollziehen, ohne mehr zu wissen" o.ae.
